...
- **cpworker** 该工具可以轻松地在网卡上抓数据包，用GRE头进行封装并发送到远端的设备，来进行数据包监控和分析。

![cpworker capture traffic flow via GRE/Cloud](./img/cpworker.png)

此外，还有工具：
- **cpdaemon** 是cpworker的管理程序。它可以拉起和停止cpworker，并且设定命令行的参数。该模块需要和CPM（Cloud Probe Mananger）.CPM提供了用户界面，可以对cpworker的策略进行配置，同时还可以图形化显示cpdaemon上报的统计数据。您可以联系Netis获取关于CPM的进一步支持，或者可以开发自己的CPM。
