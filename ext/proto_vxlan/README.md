...
## Introduce
Protocol VxLAN Plugin(or extension) is a dynamic library for port mirror export tunnel protocol along with Netis Packet Agent. 

## Getting Started
### Installation
```bash
cd ./packet-agent/ext/proto_vxlan
cmake .
make -j 4
# Then the libproto_vxlan.so binary will be generated at "./packet-agent/bin/".
```
