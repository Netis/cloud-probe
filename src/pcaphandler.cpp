...
void PcapHandler::packetHandler(const struct pcap_pkthdr *header, const uint8_t *pkt_data) {
    std::for_each(_exports.begin(), _exports.end(), [header, pkt_data, this](std::shared_ptr<PcapExportBase> pcapExport) {
...
int PcapHandler::startPcapLoop(int count) {
    if (_pcap_handle == NULL) {
        std::cerr << "pcap has not created!" << std::endl;
        return -1;
    }
...
int PcapOfflineHandler::openPcap(const std::string &dev, const pcap_init_t &param, const std::string &expression,
    ...
    if (!pcap_handle) {
        std::cerr << "pcap_open_offline failed, err: " << _errbuf << std::endl;
        return -1;
    }
    ...
    if (dumpfile) {
        if (openPcapDumper(pcap_handle) != 0) {
            std::cerr << "openPcapDumper failed!!" << std::endl;
            return -1;
        }
    }
    pcapGuard.Dismiss();
...
int PcapLiveHandler::openPcap(const std::string &dev, const pcap_init_t &param, const std::string &expression,
    ...
    if (!pcap_handle) {
        std::cerr << "pcap_create failed, err: " << _errbuf << std::endl;
        return -1;
    }
    auto pcapGuard = MakeGuard([pcap_handle]() {
        pcap_close(pcap_handle);
    ...
    if (ret != 0) {
        std::cerr << "set pcap buffer size to " << param.buffer_size << " failed." << std::endl;
        return -1;
    }
    ...
    if (expression.length() > 0) {
        std::cout << "set pcap filter!!" << std::endl;
        if (pcap_lookupnet(dev.c_str(), &net, &mask, _errbuf) ==-1 ) {
            std::cerr << "pcap_lookupnet failed, err: " << _errbuf << std::endl;
            return -1;
        }

        ret = pcap_compile(pcap_handle, &filter, expression.c_str(), 0, net);
        if (ret != 0) {
            std::cerr << "pcap_compile error: " << pcap_statustostr(ret) << std::endl;
            return -1;
        }

        ret = pcap_setfilter(pcap_handle, &filter);
        if (ret != 0) {
            std::cerr << "pcap_setfilter error: " << pcap_statustostr(ret) << std::endl;
            return -1;
        }
    }
