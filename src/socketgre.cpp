...
int PcapExportGre::initExport() {
    ...
    if (_socketfd==INVALIDE_SOCKET_FD) {
        ...
        if ((_socketfd = socket(AF_INET, SOCK_RAW, IPPROTO_GRE))==INVALIDE_SOCKET_FD) {
            std::cerr << "create socket failed! errcode: " << errno << ", strerr: " << strerror(errno) << std::endl;
            return -1;
        }
    }
    return 0;
...
int PcapExportGre::exportPacket(const struct pcap_pkthdr *header, const uint8_t *pkt_data) {
    ...
    while (nSend == -1 && errno == ENOBUFS) {
        usleep(1000);
        nSend = static_cast<int>(sendto(_socketfd, _grebuffer, length+sizeof(grehdr_t), 0, (struct sockaddr *)&_remote_addr,
                            sizeof(struct sockaddr)));
    }
    if (nSend == -1) {
        std::cerr << "socket sendto failed! errcode: " << errno << ", strerr: " << strerror(errno) << std::endl;
        return -1;
    }
    if (nSend < (ssize_t)(length+sizeof(grehdr_t))) {
        std::cerr << "socket will send " << length+sizeof(grehdr_t) << " bytes, but only " << nSend <<
            " bytes are sent successfully!" << std::endl;
        return 1;
    }
    return 0;
}
