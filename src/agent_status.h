...
public:
    int update_capture_status(uint64_t cur_pkt_time, uint64_t total_pkt_caplen,  uint64_t fwd_pkt_caplen,
                                uint64_t total_fwd_count, uint64_t total_cap_count, pcap_t* handle = NULL);
...
private:
    ...
    std::atomic<uint64_t> _total_fwd_bytes;
    std::atomic<uint64_t> _total_fwd_count;
};
